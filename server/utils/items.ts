...
const APPLY_EN: Record<number, string> = {
	...
	82: 'Energy',
	83: 'Defense',
	84: 'Costume Attr Bonus',
	85: 'Magic Attack',
	86: 'Melee+Magic Attack',
	87: 'Resist Ice',
	88: 'Resist Earth',
	89: 'Resist Dark',
	90: 'Anti-Critical',
...
const APPLY_RU: Record<number, string> = {
	...
	15: 'Крит. удар',
	16: 'Пробивание',
	17: 'Атака против Людей',
	18: 'Атака против Животных',
	19: 'Атака против Орков',
	20: 'Атака против Мистиков',
	21: 'Атака против Нежити',
	22: 'Атака против Дьяволов',
	23: 'Похищение здоровья',
	24: 'Похищение маны',
	25: 'Сжигание маны',
	26: 'Мана при уроне',
	27: 'Блок',
	...
	37: 'Сопр. магии',
	38: 'Сопр. ветру',
	39: 'Отражение ближнего боя',
	40: 'Отражение проклятий',
	41: 'Сниж. урона от яда',
	42: 'Мана за убийство',
	43: 'Бонус опыта',
	44: 'Бонус золота',
	45: 'Бонус дропа',
	46: 'Эффект зелий',
	47: 'Здоровье за убийство',
	48: 'Иммун. к оглушению',
	49: 'Иммун. к замедлению',
	50: 'Иммун. к падению',
	51: 'Навык',
	52: 'Дальность лука',
	53: 'Сила атаки',
	...
	56: 'Маг. защита',
	57: 'Шанс проклятия',
	58: 'Макс. выносливость',
	59: 'Атака против Воинов',
	60: 'Атака против Ниндзя',
	61: 'Атака против Сур',
	62: 'Атака против Шаманов',
	63: 'Атака против Монстров',
	64: 'Атака (премиум)',
	65: 'Защита (премиум)',
	66: 'Опыт (премиум)',
	67: 'Дроп (премиум)',
...
export function decodeStats(it: Row, lang: Lang): { label: string, text: string }[] {
	...
	if (it.type === 1) {
		...
		const pMin = (it.value3 || 0) + add
		const pMax = (it.value4 || 0) + add
		if (pMin > 0 || pMax > 0)
			out.push({ label: statLabel('attack', lang), text: range(pMin, pMax) })
		if ((it.value1 || 0) > 0 || (it.value2 || 0) > 0)
			out.push({ label: statLabel('magicAttack', lang), text: range((it.value1 || 0) + add, (it.value2 || 0) + add) })
	}
	else if (it.type === 2) {
		const def = (it.value1 || 0) + (it.value5 || 0) * 2
		if ((it.value1 || 0) > 0)
			out.push({ label: statLabel('defense', lang), text: String(def) })
		if ((it.value0 || 0) > 0)
			out.push({ label: statLabel('magicDefense', lang), text: String(it.value0) })
	}
	return out
}
...
export function appliesIndexed(it: Row, lang: Lang): { idx: number, type: number, label: string, value: number, percent: boolean }[] {
	const out = []
	for (let i = 0; i < 3; i++) {
		const t = it[`applytype${i}`]
		const v = it[`applyvalue${i}`]
		if (t && v)
			out.push({ idx: i, type: t, label: applyLabel(t, lang), value: v, percent: PERCENT_APPLIES.has(t) })
	}
	return out
}
...
export function describeLimits(it: Row, lang: Lang): { label: string, value: number }[] {
	const out = []
	for (let i = 0; i < 2; i++) {
		const t = it[`limittype${i}`]
		const v = it[`limitvalue${i}`]
		if (t === 1 && v > 0)
			out.push({ label: statLabel('level', lang), value: v })
	}
	return out
}
