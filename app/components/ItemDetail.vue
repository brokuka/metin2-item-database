...
<template>
	...
	<div v-else-if="!item || !current" class="py-8 text-center text-muted">{{ t('items.none') }}</div>
	...
	<div v-else>
		<div class="flex items-start gap-4">
			...
			<div class="min-w-0 flex-1">
				<h1 class="text-xl font-bold text-default">{{ item.name }}</h1>
		...
		<div class="mt-4 rounded-lg bg-elevated/40 p-3">
			<p class="mb-1 text-xs font-semibold uppercase text-muted">{{ t('items.description') }}</p>
		...
		<div v-if="levelItems.length > 1" class="mt-4">
			<p class="mb-1 text-xs font-semibold uppercase text-muted">{{ t('items.refineLevel') }}</p>
		...
		<div v-if="current.stats?.length" class="mt-4">
			<p class="mb-1 text-xs font-semibold uppercase text-muted">{{ t('items.stats') }}</p>
			<ul class="space-y-1 text-sm">
				<li v-for="(s, i) in current.stats" :key="i" class="text-default">{{ s.label }}: <span class="font-medium">{{ s.text }}</span></li>
			</ul>
		</div>

		<div v-if="current.limits?.length" class="mt-4">
			<p class="mb-1 text-xs font-semibold uppercase text-muted">{{ t('items.requirements') }}</p>
			<ul class="space-y-1 text-sm">
				<li v-for="(l, i) in current.limits" :key="i" class="text-default">{{ l.label }}: {{ l.value }}</li>
			</ul>
		</div>

		<div v-if="item.bonuses?.length" class="mt-4">
			<p class="mb-1 text-xs font-semibold uppercase text-muted">{{ t('items.bonuses') }}</p>
			<ul class="space-y-1 text-sm">
